...
- 렌더 상태는 래스터화기 상태, 블렌딩 상태, 깊이스텐실 상태가 있다.
...
- 역전치 행렬 : 접선 벡터 u = v1 - v0에 비균등 비례 행렬( A )을 곱하면 노말 벡터와 수직을 이루지 않는다. 그래서 노말 벡터에
A의 역행렬의 전치 행렬, 즉 역전치행렬을 곱함으로서 변환된 접선 벡터의 수직인 노말 벡터를 구할 수 있다.

- 역전치의 배경
<div>
 <img src="Image/역전치.PNG" width = 600 height = 300/>
</div>

- 역전치 과정
<div>
 <img src="Image/역전치 그래프.PNG" width = 400 height = 800/>
</div>

### 반영광 조명

- 반사 벡터 : r = I - 2(n dot I)n ( I : 입사 벡터, n : 노말 벡터 )

### 구현

- 상수 버퍼를 위한 구조체를 생성할 때, HLSL의 하나의 멤버는 한 벡터에 들어가야 된다는
 채우기 규칙을 지켜야 한다.
 
- 배열의 경우 여분의 공간이 있어도 각각의 원소가 하나의 벡터를 갖는다.

```hlsl
// 명시적 형변환과 주소 계산 명령으로 메모리 효율을 높일 수 있다.
float4 array[4];
