...
Texture2D g_DiffuseTex_2 : register (t1);

Texture2D g_NormalMapTex : register (t2);

Texture2D g_NormalMapTex_2 : register (t3);
...
DomainOut DS(PatchTess patchTess, float3 bary : SV_DomainLocation, const OutputPatch<HullOut, 3> tri)
{
	...
	o.uv2 = bary.x * tri[0].uv2 + bary.y * tri[1].uv2 + bary.z * tri[2].uv2;
...
float4 ps_main(DomainOut i) : SV_Target
{
	...
	float4 texColor = float4(0.4, 0.6, 1, 1);
	...
	float3 tangentN2 = g_NormalMapTex_2.Sample(smpLinear, i.uv2).xyz;

	tangentN2 = tangentN2 * 2.f - 1.f;

	float3x3 TBN = float3x3(T, B, N);

	N = normalize(mul(tangentN, TBN) + mul(tangentN2, TBN));
